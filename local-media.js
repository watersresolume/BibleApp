...
class LocalMediaHandler {
    ...
    async loadMediaFromFolder() {
        ...
        try {
            ...
            // Use the Node.js fs module to read the directory
            const fs = require('fs');
            const path = require('path');
            
            // Read directory contents
            const files = fs.readdirSync(this.mediaFolderPath)
                .filter(file => {
                    const ext = path.extname(file).toLowerCase();
                    return ['.jpg', '.jpeg', '.png', '.gif', '.mp4', '.mp3', '.pdf'].includes(ext);
                })
                .map(file => {
                    const filePath = path.join(this.mediaFolderPath, file);
                    const stats = fs.statSync(filePath);
                    return {
                        name: file,
                        path: filePath,
                        size: stats.size,
                        type: this.getFileType(path.extname(file).slice(1))
                    };
                });
            ...
            files.forEach(file => {
                ...
                mediaCard.dataset.mediaType = file.type;
                ...
                mediaCard.innerHTML = `
                    <div class="card-content">
                        <div class="card-thumbnail">
                            ${file.type === 'image' 
                                ? `<img src="file://${file.path}" alt="${file.name}" onerror="this.parentElement.innerHTML='${this.getFileIcon(file.type)}'">`
                                : `<div class="card-icon">${this.getFileIcon(file.type)}</div>`
                            }
                        </div>
                        <div class="card-title" title="${file.name}">${this.truncateFileName(file.name)}</div>
                        <div class="card-subtitle">${file.type} • ${this.formatFileSize(file.size)}</div>
                    </div>
                `;
    ...
    makeMediaItemInteractive(item) {
        ...
        const handleMouseDown = (e) => {
            ...
            startX = e.clientX;
            startY = e.clientY;
            itemX = parseInt(item.style.left);
